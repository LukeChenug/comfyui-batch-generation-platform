...
- [☁️ ComfyUI Deploy 云端故障](#comfyui-deploy-云端故障)
...
1. **ComfyUI服务器地址配置**
   ```python
   # 编辑 backend/src/config/settings.py
   # 或者直接设置环境变量
...
## ☁️ ComfyUI Deploy 云端故障

### 症状
- 报错 `SSLCertVerificationError` 或 `CERTIFICATE_VERIFY_FAILED`。
- 云端任务状态为 `success`，但图片未下载。

### 根本原因
- 本地 Python 环境缺少受信任的根证书 (CA Certificates)，导致无法连接 `api.comfydeploy.com`。
- ComfyDeploy 返回的 JSON 结构复杂，包含多层嵌套，导致解析失败。

### ⚠️ 临时解决方案 (当前版本已应用)
我们在代码中使用了**强制绕过 SSL 验证**的方案：
1.  在 API 请求时，设置 `verify=False` (requests) 或 `ssl=False` (aiohttp)。
2.  这虽然能解决连接问题，但在**生产环境**是不安全的。

### ✅ 待办优化 (TODO)
为了系统安全，建议在后续版本中：
1.  **Mac 用户**：运行 Python 安装目录下的 `Install Certificates.command`。
2.  **Linux/Docker**：确保系统安装了 `ca-certificates` 包。
3.  **代码层**：移除硬编码的 `ssl=False`，改为通过环境变量 `SSL_VERIFY=True/False` 控制。
...
检查1：API服务器是否运行？
    ├─ 是 → 检查2
    └─ 否 → 启动服务器：./quick_start.sh 或 python -m backend.src.main
            ↓
检查2：API地址是否正确？
    ├─ 是 → 检查3
    └─ 否 → 修改Web界面中的API地址
